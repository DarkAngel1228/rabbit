...
import com.imooc.task.annotation.ElasticJobConfig;
import com.imooc.task.autoconfigure.JobZookeeperProperties;
import com.imooc.task.enums.ElasticJobTypeEnum;
...
public class ElasticJobConfParser implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        try {
            ...
            for (Iterator<?> it = beanMap.values().iterator(); it.hasNext();) {
                ...
                // 接下来就是把factory 也就是SpringJobScheduler注入到Spring容器中
    ...
    private List<BeanDefinition> getTargetElasticJobListeners(ElasticJobConfig conf) {
        ...
        if (StringUtils.isNotBlank(distributedListener)) {
